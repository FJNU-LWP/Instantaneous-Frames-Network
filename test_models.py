...
def eval_video(video_data, net, this_test_segments, modality):
    ...
    with torch.no_grad():
        ...
        if is_shift:
            ...
            #print(data_in.size())
            #print(batch_size )
            data_in = data_in#data_in.view(batch_size * num_crop, this_test_segments, length, data_in.size(2), data_in.size(3))
            #print(data_in)
        rst = net(data_in)
        #print(rst.shape)
        #print(label.shape)
...
for i, data_label_pairs in enumerate(zip(*data_iter_list)):
    with torch.no_grad():
        ...
        for n_seg, (_, (data, label)), net, modality in zip(test_segments_list, data_label_pairs, net_list, modality_list):
            ...
            j=+1
            print(j)
            #print('this_rst_list',this_rst_list.shape)
            #breakpoint()
            this_label = label
        ############
        prec1 = gaze_accuracy(rst[1], this_label)
        prec5=prec1
        top1.update(prec1, this_label.numel())
        top5.update(prec5, this_label.numel())

        #################
print("平均准确率:", top1.avg)
breakpoint()
'''
