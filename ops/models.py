...
class TSN(nn.Module):
    ...
    def forward(self, input, no_reshape=False):
        if self.modality == 'Flow':  # wu
            input=input[:,0:10,:,:]
            x = input.contiguous().view((-1, 10) + input.size()[-2:])
            x1, x2, x3, x4, x5 = x[:, 0:2, :, :], x[:, 2:4, :, :], x[:, 4:6, :, :], x[:, 6:8, :, :], x[:, 8:10, :, :]
            ###########
            if self.num_segments== 5:
                input1 = torch.cat([x3 - x1, x3 - x1, x3 - x1, x3 - x1, x3 - x1], 1).view(-1, 10, x3.size()[2], x3.size()[3])
                input2 = torch.cat([x3 - x2, x3 - x2, x3 - x2, x3 - x2, x3 - x2], 1).view(-1, 10, x3.size()[2], x3.size()[3])
                input3 = torch.cat([x3, x3, x3, x3, x3], 1).view(-1, 10, x3.size()[2], x3.size()[3])
                input4 = torch.cat([x3 - x4, x3 - x4, x3 - x4, x3 - x4, x3 - x4], 1).view(-1, 10, x3.size()[2], x3.size()[3])
                input5 = torch.cat([x3 - x5, x3 - x5, x3 - x5, x3 - x5, x3 - x5], 1).view(-1, 10, x3.size()[2], x3.size()[3])
                input = torch.cat([input1, input2, input3, input4, input5], 1).view(-1, 50, x3.size()[2], x3.size()[3])

            ######
            else:
                input=x
                #input = torch.cat([x3-x1, x3 - x2, x3 , x3 - x4, x3 - x5], 1).view(-1, 10, x3.size()[2], x3.size()[3])
        else:
            input = input[:, 0:15, :, :]
            x = input.contiguous().view((-1, 15) + input.size()[-2:])
            x1, x2, x3, x4, x5 = x[:, 0:3, :, :], x[:, 3:6, :, :], x[:, 6:9, :, :], x[:, 9:12, :, :], x[:, 12:15, :, :]
            #input = torch.cat([x3, x2 - x1, x3 - x2, x4 - x3, x5 - x4], 1).view(-1, 15, x3.size()[2], x3.size()[3])
            input = torch.cat([x3-x1, x3 - x2, x3 , x3 - x4, x3 - x5], 1).view(-1, 15, x3.size()[2], x3.size()[3])
